                ...
                */
                document.querySelectorAll('*[data-cites]').forEach(e =>{
                    ...
                    const writeAriaLabel = (bibTarget)=>{
                        ...
                        return `${bibTarget.children[0].innerHTML.match(onlyDigits)}, ${bibTarget.children[1].innerHTML.substring(0,bibTarget.children[1].innerHTML.indexOf(',')).replace('<em>et al.</em>','et al.').replace(/(\r\n|\n|\r)/gm,' ')}`
                    }
                    const lastSentence = () => {
                        ...
                        const sentences = e.previousSibling.wholeText.replace(/(\r\n|\n|\r)/gm,' ').replace(/([.?!])\s*(?=[A-Z])/g, "$1|").split("|")
